...
const StaffComponent = () => {
  ...
  const addStaff = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setNewStaff({ staffName: '', staffRole: '', staffEmail: '', staffMobile: '', staffPassword: '' });
        setOpenAddDialog(false);
        setSuccessMessage('Staff added successfully!');
      } else {
        setErrorMessage(result.message || 'Failed to add staff.');
      }
    } catch (error) {
      console.error('Error adding staff:', error);
      setErrorMessage('Server error while adding staff.');
    }
  };
  ...
  return (
    <Box sx={{ padding: '20px', backgroundColor: colors.light, borderRadius: '8px' }}>
      ...
      <List>
        {staff.filter(member => member.staffName.toLowerCase().includes(searchQuery.toLowerCase())).map(member => (
